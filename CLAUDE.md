...
- Triggered by Scheduler at **03:00 Rome** via `GET /api/trigger`, auth `Authorization: Bearer` (or `X-Trigger-Secret`); never query-string.
- `/api/trigger` returns **immediate `202`**, sync runs in a background thread (**~212s**). It is a *background* thread, so gunicorn's `--timeout 120` does not apply to it — only to the request, which returns at once.
- `/api/status` to check last execution status — in-memory: reset on every deploy/restart; proof of a run = Render logs.
